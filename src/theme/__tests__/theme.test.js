...
describe('theme', () => {
  ...
  describe('lightTheme', () => {
    ...
    test('should use manila colors appropriately', () => {
      ...
      expect(lightTheme.colors.background).toBe(manyllaColors.lightManilaBackground);
    ...
    test('should use Atkinson Hyperlegible font', () => {
      expect(lightTheme.typography.fontFamily).toContain('Atkinson Hyperlegible');
    });
  });
  ...
  describe('darkTheme', () => {
    ...
    test('should have dark color scheme', () => {
      ...
      expect(darkTheme.colors.textSecondary).toBe(manyllaColors.darkTextSecondary);
    });
  ...
  describe('manyllaTheme', () => {
    ...
    test('should use manylla color scheme', () => {
      expect(manyllaTheme.colors.background).toBe(manyllaColors.manyllaBackground);
      ...
      expect(manyllaTheme.colors.textSecondary).toBe(manyllaColors.manyllaTextSecondary);
    });
    ...
    test('should have manila-specific shadow styling', () => {
      expect(manyllaTheme.components.card.shadowColor).toBe('rgba(61, 47, 31, 0.15)');
  ...
  describe('theme consistency', () => {
    test('all themes should have the same typography structure', () => {
      const expectedTypographyKeys = ['fontFamily', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'body1', 'body2', 'caption'];
    ...
    test('all themes should have the same shape configuration', () => {
      ...
      expect(manyllaTheme.shape.borderRadius).toBe(lightTheme.shape.borderRadius);
    });

    test('all themes should have required color properties', () => {
      const requiredColorKeys = [
        'primary', 'primaryLight', 'primaryDark', 'secondary',
        'background', 'surface', 'text', 'textSecondary', 'textDisabled',
    ...
    test('typography sizes should be reasonable', () => {
      ...
      themes.forEach(theme => {
        expect(theme.typography.h1.fontSize).toBeGreaterThan(theme.typography.h2.fontSize);
        expect(theme.typography.h2.fontSize).toBeGreaterThan(theme.typography.body1.fontSize);
        expect(theme.typography.body1.fontSize).toBeGreaterThan(theme.typography.caption.fontSize);
