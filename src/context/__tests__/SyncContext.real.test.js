  ...
  return (
    <div>
      <div data-testid="sync-enabled">
        {syncContext.syncEnabled ? "enabled" : "disabled"}
      </div>
      <div data-testid="sync-status">{syncContext.syncStatus}</div>
      <div data-testid="recovery-phrase">
        {syncContext.recoveryPhrase || "none"}
      </div>
      <div data-testid="sync-id">{syncContext.syncId || "none"}</div>
      <div data-testid="last-pull">
        {syncContext.lastSync ? syncContext.lastSync.toString() : "never"}
      </div>
      ...
      <button
        ...
        onClick={() =>
          syncContext.pushSync(testActions.testData || { test: "data" })
        }
      >
        Push Data
      </button>

      <button data-testid="pull-data" onClick={() => syncContext.pullSync()}>
        Pull Data
      </button>
...
describe("SyncContext Real Integration", () => {
  ...
  describe("Initial State and Provider Setup", () => {
    test("should provide initial sync state", () => {
      ...
      render(
        <SyncProvider>
          <TestSyncConsumer
            onContextUpdate={(ctx) => (capturedContext = ctx)}
          />
        </SyncProvider>,
      );
  ...
  describe("Sync Enable/Disable Workflow", () => {
    test("should complete sync enablement workflow", async () => {
      ...
      expect(screen.getByTestId("sync-status")).not.toHaveTextContent(
        "not-setup",
      );
    ...
    test("should handle sync disable", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId("sync-enabled")).toHaveTextContent(
          "disabled",
        );
      });
  ...
  describe("State Persistence", () => {
    test("should persist sync state across provider remounts", async () => {
      ...
      await waitFor(() => {
        ...
        expect(newSyncId === "none" || newSyncId === syncId).toBe(true);
      });
    });
    ...
    test("should handle localStorage errors gracefully", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId("recovery-phrase")).not.toHaveTextContent(
          "none",
        );
      });
