...
describe('Validation Real Integration Tests', () => {
  describe('Profile Validation', () => {
    ...
    test('should validate profile with extensive data', () => {
      const extensiveProfile = createValidProfile({
        entries: [
          ...
          createValidEntry({ id: 'entry-2', title: 'Educational Entry', category: 'education' }),
        ],
        categories: [
    ...
    test('should validate profile name constraints', () => {
      // Too short
      let result = ProfileValidator.validateProfile(createValidProfile({ name: 'A' }));
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('Profile name must be at least 2 characters');

      // Too long
      result = ProfileValidator.validateProfile(createValidProfile({
      ...
      // Valid length
      result = ProfileValidator.validateProfile(createValidProfile({ name: 'Valid Name' }));
      expect(result.valid).toBe(true);
    });

    test('should validate entries array', () => {
      // Null entries
      let result = ProfileValidator.validateProfile(createValidProfile({ entries: null }));
      expect(result.valid).toBe(false);
...
*Important*: Remember to bring insurance card.`
    ...
    test('should validate entry with attachments', () => {
      const entryWithAttachments = createValidEntry({
        attachments: [
          { name: 'medical-report.pdf', type: 'application/pdf', size: 1024000 },
    ...
    test('should validate title length constraints', () => {
      // Too long
      let result = ProfileValidator.validateEntry(createValidEntry({
      ...
      // Valid length
      result = ProfileValidator.validateEntry(createValidEntry({ title: 'Valid Title' }));
      expect(result.valid).toBe(true);
    });
    ...
    test('should validate description length', () => {
      ...
      const result = ProfileValidator.validateEntry(createValidEntry({
    ...
    test('should validate date formats', () => {
      // Invalid date
      let result = ProfileValidator.validateEntry(createValidEntry({ date: 'invalid-date' }));
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('Invalid date format');

      // Future date
      result = ProfileValidator.validateEntry(createValidEntry({
    ...
    test('should validate visibility settings', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.some(error => error.includes('visibility'))).toBe(true);
  ...
  describe('Category Validation', () => {
    ...
    test('should validate color format', () => {
      ...
      let result = ProfileValidator.validateCategory(createValidCategory({
        color: 'invalid-color'
      }));
      expect(result.valid).toBe(false);
      ...
      // Valid color formats
      result = ProfileValidator.validateCategory(createValidCategory({ color: '#FF0000' }));
      expect(result.valid).toBe(true);

      result = ProfileValidator.validateCategory(createValidCategory({ color: '#f00' }));
      expect(result.valid).toBe(true);

      result = ProfileValidator.validateCategory(createValidCategory({ color: 'red' }));
      expect(result.valid).toBe(true);
    });
    ...
    test('should validate category name constraints', () => {
      ...
      // Special characters
      result = ProfileValidator.validateCategory(createValidCategory({
  ...
  describe('Input Sanitization', () => {
    ...
    test('should preserve safe HTML in markdown', () => {
      ...
      const sanitized = ProfileValidator.sanitizeInput(markdownInput, { allowMarkdown: true });
...
[Bad link](javascript:alert('XSS'))`;
  ...
  describe('Form Data Validation', () => {
    ...
    test('should provide detailed error paths for nested validation', () => {
      ...
      expect(result.errors.some(error => error.includes('profiles[0]'))).toBe(true);
      expect(result.errors.some(error => error.includes('profiles[1]'))).toBe(true);
    });
  });
  ...
  describe('Performance and Edge Cases', () => {
    ...
    test('should handle circular references safely', () => {
      ...
      expect(() => ProfileValidator.validateProfile(circularProfile)).not.toThrow();
    });
