...
export const validateImage = (file) => {
  ...
  if (file.type && file.fileSize) {
    ...
    if (file.fileSize > IMAGE_CONFIG.MAX_FILE_SIZE) {
      return { isValid: false, error: `File size too large. Maximum ${IMAGE_CONFIG.MAX_FILE_SIZE / 1024 / 1024}MB allowed.` };
    }
 ...
 */
export const calculateDimensions = (width, height, maxDimension = IMAGE_CONFIG.MAX_DIMENSION) => {
 ...
 */
export const resizeImage = (dataUrl, maxDimension = IMAGE_CONFIG.MAX_DIMENSION) => {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      try {
        ...
        const quality = dataUrl.toLowerCase().includes('png') ? 0.9 : IMAGE_CONFIG.JPEG_QUALITY;
