...
describe("ManyllaEncryptionService Integration Tests", () => {
  ...
  beforeAll(() => {
    if (isRealApi) {
      console.log(
        `Running integration tests against ${testConfig.env} API: ${testConfig.apiUrl}`,
      );
    } else {
      console.log(
        "Running tests in mock mode (set API_ENV=dev to test against real API)",
      );
    }
  });
  ...
  describe("API Integration Tests", () => {
    ...
    runIfRealApi(
      "should push and pull encrypted data",
      async () => {
    ...
    runIfRealApi(
      "should handle concurrent sync operations",
      async () => {
        const phrase = service.generateRecoveryPhrase();
        await service.init(phrase);
        const syncId = service.syncId;
  ...
  describe("Storage Integration", () => {
    test("should persist and restore encryption state", async () => {
      ...
      AsyncStorage.getItem.mockImplementation((key) => {
        if (key === "secure_manylla_sync_id")
          return Promise.resolve(phrase.slice(0, 16));
  ...
  describe("Performance Tests", () => {
    test("should handle encryption within performance limits", async () => {
      ...
      for (const size of sizes) {
        ...
        console.log(
          `${size.name}: encrypt=${encryptTime}ms, decrypt=${decryptTime}ms`,
        );
