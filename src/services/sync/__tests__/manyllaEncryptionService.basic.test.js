...
import service from '../manyllaEncryptionService';
...
describe('ManyllaEncryptionService (Basic Coverage)', () => {
  ...
  describe('public API methods', () => {
    ...
    test('encrypt methods throw when not initialized', () => {
      expect(() => service.encryptData({})).toThrow('Encryption not initialized');
      expect(() => service.encrypt({})).toThrow('Encryption not initialized');
    });

    test('decrypt methods throw when not initialized', () => {
      expect(() => service.decryptData('data')).toThrow('Encryption not initialized');
      expect(() => service.decrypt('data')).toThrow('Encryption not initialized');
    });
  ...
  describe('key derivation edge cases', () => {
    ...
    test('produces different results for different phrases', async () => {
      ...
      let callCount = 0;
      util.encodeBase64.mockImplementation((bytes) => `mock${callCount++}${bytes.length}`);
