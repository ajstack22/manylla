...
if (Platform.OS === 'web') {
  MUIIcons = {
    ...
    CheckBoxOutlineBlank: require('@mui/icons-material/CheckBoxOutlineBlank').default,
    RadioButtonChecked: require('@mui/icons-material/RadioButtonChecked').default,
    RadioButtonUnchecked: require('@mui/icons-material/RadioButtonUnchecked').default,
...
const Icon = ({ name, size = 24, color = '#000000', style, ...props }) => {
  if (Platform.OS === 'web' && MUIIcons[name]) {
    ...
    return <IconComponent style={{ fontSize: size, color, ...style }} {...props} />;
  }
  
  if (Platform.OS !== 'web' && MaterialIcons && iconNameMap[name]) {
    return <MaterialIcons name={iconNameMap[name]} size={size} color={color} style={style} {...props} />;
  }
  
  // Fallback to text when icon library not available
  console.warn(`Icon not available: ${name}, MaterialIcons loaded: ${!!MaterialIcons}`);
