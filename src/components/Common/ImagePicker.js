...
export class ImagePicker {
  ...
  static capturePhoto(options = {}) {
    if (!platform.isMobile) {
      return Promise.reject(
        new Error("Camera capture only available on mobile devices"),
      );
    }
  ...
  static _selectImageMobile(source, options) {
    ...
    if (!picker) {
      return Promise.reject(
        new Error("Image picker not available on this platform"),
      );
    }
  ...
  static validateResult(result) {
    ...
    if (
      result.type &&
      !IMAGE_CONFIG.ALLOWED_TYPES.includes(result.type.toLowerCase())
    ) {
      return {
        isValid: false,
