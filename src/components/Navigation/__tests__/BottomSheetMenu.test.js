...
jest.mock('../../Common', () => ({
  ShareIcon: ({ size, color, style }) => (
    <span data-testid="share-icon" style={style}>📤</span>
  ),
  PrintIcon: ({ size, color, style }) => (
    <span data-testid="print-icon" style={style}>🖨️</span>
  ),
  CloudIcon: ({ size, color, style }) => (
    <span data-testid="cloud-icon" style={style}>☁️</span>
  ),
  PaletteIcon: ({ size, color, style }) => (
    <span data-testid="palette-icon" style={style}>🎨</span>
  ),
  PrivacyTipIcon: ({ size, color, style }) => (
    <span data-testid="privacy-icon" style={style}>🔒</span>
  ),
  SupportIcon: ({ size, color, style }) => (
    <span data-testid="support-icon" style={style}>❓</span>
  ),
  LogoutIcon: ({ size, color, style }) => (
    <span data-testid="logout-icon" style={style}>🚪</span>
  ),
  CloseIcon: ({ size, color, style }) => (
    <span data-testid="close-icon" style={style}>✕</span>
  ),
}));

// Mock Material-UI icons
jest.mock('@mui/icons-material/ExpandLess', () => ({
  default: ({ style }) => <span data-testid="expand-less" style={style}>▲</span>
}));

jest.mock('@mui/icons-material/ExpandMore', () => ({
  default: ({ style }) => <span data-testid="expand-more" style={style}>▼</span>
}));
...
describe('BottomSheetMenu', () => {
  ...
  describe('More Menu Functionality', () => {
    ...
    it('should call showToast when theme is toggled', () => {
      ...
      expect(mockProps.showToast).toHaveBeenCalledWith('Dark mode activated', 'info');
    });
    ...
    it('should handle close profile action with danger styling', () => {
      ...
      // Click Close Profile
      fireEvent.click(screen.getByText('Close Profile').closest('button'));
  ...
  describe('Accessibility', () => {
    it('should have proper accessibility labels', () => {
      ...
      expect(shareButton).toHaveAttribute('accessibilityLabel', 'Share profile information');

      const printButton = screen.getByText('Print').closest('button');
      expect(printButton).toHaveAttribute('accessibilityLabel', 'Print profile information');

      const syncButton = screen.getByText('Sync').closest('button');
      expect(syncButton).toHaveAttribute('accessibilityLabel', 'Sync data across devices');
    });
  ...
  describe('Theme Support', () => {
    ...
    it('should show correct next theme name in toast', () => {
      // Test light -> dark
      const { unmount } = render(<BottomSheetMenu {...mockProps} theme="light" />);
      fireEvent.click(screen.getAllByText('More')[0].closest('button'));
      fireEvent.click(screen.getByText('Theme').closest('button'));
      expect(mockProps.showToast).toHaveBeenCalledWith('Dark mode activated', 'info');
      unmount();
      ...
      fireEvent.click(screen.getAllByText('More')[0].closest('button'));
      fireEvent.click(screen.getByText('Theme').closest('button'));
      expect(mockProps.showToast).toHaveBeenCalledWith('Manylla mode activated', 'info');
    });
  });
