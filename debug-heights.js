...
(() => {
  ...
  const profileCard =
    document.querySelector('[class*="profileCardDesktop"]') ||
  ...
  const quickInfo = Array.from(document.querySelectorAll("div")).find(
    (el) =>
  ...
  if (profileCard && quickInfo) {
    ...
    console.log(
      "HEIGHT DIFFERENCE:",
      profileRect.height - quickRect.height,
      "px",
    );
    console.log("=================================");
    ...
    const possibleProfile = allDivs.find(
      (el) =>
        el.textContent.includes("Ellie") && el.textContent.includes("Age:"),
    );
    ...
    if (possibleQuick) {
      ...
      possibleQuick.parentElement.parentElement.style.outline =
        "3px dashed green";
    }
  }
})();
