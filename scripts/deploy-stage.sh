...
# Manylla Stage Deployment Script - Enhanced Security & Quality Version
...
echo "🚀 Manylla Stage Deployment - Enhanced Validation Mode"
echo "===================================================="
echo
...
if [ -f .env.stage ]; then
    ...
    echo -e "${YELLOW}⚠️  No .env.stage file found, using defaults${NC}"
fi

# Update homepage in package.json for stage subdirectory
...
echo -e "${BLUE}Deploying to Stage Server${NC}"
echo "─────────────────────────"
...
echo -e "${YELLOW}🧹 Cleaning stage directory...${NC}"
ssh stackmap-cpanel "
    cd ~/public_html/manylla/stage && \
    find . -type f ! -name '.htaccess' ! -name '*.php' ! -path './api/config/*' -delete && \
    find . -type d -empty -delete 2>/dev/null || true
" || handle_error "Failed to clean stage directory" \
    "Check SSH connection and permissions"

echo -e "${GREEN}✅ Stage directory cleaned${NC}"

# Deploy build files
echo -e "${YELLOW}🚀 Deploying to staging...${NC}"
ssh stackmap-cpanel "mkdir -p ~/public_html/manylla/stage"

rsync -avz --delete \
    --exclude='.git' \
    --exclude='node_modules' \
    --exclude='.env' \
    web/build/ stackmap-cpanel:~/public_html/manylla/stage/ || \
...
if [ -f "public/.htaccess.manylla-stage" ]; then
    echo -e "${YELLOW}📄 Deploying Manylla stage .htaccess...${NC}"
    scp public/.htaccess.manylla-stage stackmap-cpanel:~/public_html/manylla/stage/.htaccess
...
if [ -d "api" ]; then
    ...
    ssh stackmap-cpanel "mkdir -p ~/public_html/manylla/stage/api/{config,sync,share,logs}"
    
    rsync -avz \
        --exclude='.git' \
        --exclude='config/*.php' \
        --exclude='*.example.php' \
        api/sync/ stackmap-cpanel:~/public_html/manylla/stage/api/sync/
    
    scp api/config/database.php stackmap-cpanel:~/public_html/manylla/stage/api/config/
    scp api/config/config.php stackmap-cpanel:~/public_html/manylla/stage/api/config/
    
    if [ -f "api/config/config.stage.php" ]; then
        echo -e "${YELLOW}📄 Deploying stage configuration...${NC}"
        scp api/config/config.stage.php stackmap-cpanel:~/public_html/manylla/stage/api/config/
        ssh stackmap-cpanel "chmod 600 ~/public_html/manylla/stage/api/config/config.stage.php"
    else
        handle_error "Stage API config not found" \
            "Run: ./scripts/deploy-api-config.sh to create config.stage.php"
    fi
    
    ssh stackmap-cpanel "chmod 755 ~/public_html/manylla/stage/api/logs"
...
HEALTH_CHECK=$(curl -s -o /dev/null -w "%{http_code}" https://manylla.com/stage/)
...
# API health check
if [ -d "api" ]; then
    API_HEALTH=$(curl -s -o /dev/null -w "%{http_code}" https://manylla.com/stage/api/sync_health.php)
...
if [ -n "$IOS_SIMULATORS" ]; then
    ...
    echo -e "${YELLOW}🌐 Opening stage URL in simulator...${NC}"
    xcrun simctl openurl booted https://manylla.com/stage 2>/dev/null || true
...
echo -e "${GREEN}Title: $RELEASE_TITLE${NC}"
echo -e "${GREEN}Web URL: https://manylla.com/stage${NC}"
echo
echo "Deployment Summary:"
echo "─────────────────"
echo "✅ All validation checks passed"
echo "✅ Code committed and pushed to GitHub"
echo "✅ Web application deployed to stage"
if [ -n "$IOS_SIMULATORS" ]; then
...
echo "Next Steps:"
echo "──────────"
echo "1. Test the deployment at https://manylla.com/stage"
