...
# Manylla Mobile BETA Deployment Script
# Purpose: Deploy to TestFlight External Testing / Play Console Closed Testing
# Includes version management, validation, and git operations

set -e  # Exit on error
set -o pipefail  # Fail on pipe errors

echo "🚀 Manylla Mobile BETA Deployment"
echo "===================================="
echo
...
calculate_versions() {
    ...
    MONTHLY_COUNT=$(git tag --list "mobile-v${YEAR}.${MONTH}.*-beta-*" 2>/dev/null | \
...
# Step 1: Check git status (must be clean for BETA)
echo -e "${BLUE}Step 1: Git Status Check${NC}"
echo "─────────────────────────"
GIT_STATUS=$(git status --porcelain)
if [[ -n "$GIT_STATUS" ]]; then
    handle_error "Git working directory not clean" \
        "Commit or stash all changes before BETA deployment. Run: git status"
fi
...
if [ "$TODO_COUNT" -gt "20" ]; then
    show_warning "Too many TODOs: $TODO_COUNT (recommended max: 20 for BETA)"
else
...
if [ "$CONSOLE_COUNT" -gt "5" ]; then
    show_warning "Too many console.log statements: $CONSOLE_COUNT (recommended max: 5 for BETA)"
else
...
# Step 7: Release Notes Check (BETA requires release notes)
...
if [ ! -f "$RELEASE_NOTES_FILE" ]; then
    handle_error "Release notes not found" \
        "Create $RELEASE_NOTES_FILE before BETA deployment"
fi
...
# Screenshot reminder
echo
echo -e "${YELLOW}⚠️  Screenshot Reminder for BETA:${NC}"
echo "   iOS: 6.7\", 6.5\", 5.5\", iPad Pro"
echo "   Android: Phone, 7\" tablet, 10\" tablet"
echo
read -p "Continue to BETA deployment? (y/N): " -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
...
if [[ "$PLATFORM" == "ios" || "$PLATFORM" == "both" ]]; then
    echo -e "${CYAN}═══════════════════════════════════════════════════════${NC}"
    echo -e "${CYAN}    iOS BETA BUILD & UPLOAD                          ${NC}"
    echo -e "${CYAN}═══════════════════════════════════════════════════════${NC}"
    echo

    echo -e "${YELLOW}📱 Building and uploading iOS to TestFlight External...${NC}"
    ...
    echo -e "${GREEN}✅ iOS uploaded to TestFlight External${NC}"
    echo
fi
...
if [[ "$PLATFORM" == "android" || "$PLATFORM" == "both" ]]; then
    echo -e "${CYAN}═══════════════════════════════════════════════════════${NC}"
    echo -e "${CYAN}    ANDROID BETA BUILD & UPLOAD                       ${NC}"
    echo -e "${CYAN}═══════════════════════════════════════════════════════${NC}"
    echo

    echo -e "${YELLOW}📱 Building and uploading Android to Play Console Closed...${NC}"
    ...
    echo -e "${GREEN}✅ Android uploaded to Play Console Closed${NC}"
    echo
fi
...
# Commit
COMMIT_MSG="chore: Mobile BETA v${MARKETING_VERSION} (build ${IOS_BUILD_NUMBER})

Deployed to TestFlight External / Play Console Closed
...
# Create git tag
TAG_NAME="mobile-v${MARKETING_VERSION}-beta-${IOS_BUILD_NUMBER}"
git tag -a "$TAG_NAME" -m "Mobile BETA release v${MARKETING_VERSION} (build ${IOS_BUILD_NUMBER})"
...
if [[ "$PLATFORM" == "ios" || "$PLATFORM" == "both" ]]; then
    ...
    echo "  📱 TestFlight External Testing group will receive notification"
    echo
fi
...
if [[ "$PLATFORM" == "android" || "$PLATFORM" == "both" ]]; then
    ...
    echo "  📱 Play Console Closed Testing track is live"
    echo
fi

echo "Next Steps:"
echo "──────────"
echo "1. Collect feedback from BETA with internal team"
