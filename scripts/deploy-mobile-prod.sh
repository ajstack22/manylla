...
# Manylla Mobile PROD Deployment Script
# Purpose: Deploy to App Store Testing / Play Store Production Testing
# Includes version management, validation, and git operations

set -e  # Exit on error
set -o pipefail  # Fail on pipe errors

echo "🚀 Manylla Mobile PROD Deployment"
echo "===================================="
echo
...
calculate_versions() {
    ...
    MONTHLY_COUNT=$(git tag --list "mobile-v${YEAR}.${MONTH}.*-prod-*" 2>/dev/null | \
...
# Step 1: Check git status (must be clean for PROD)
echo -e "${BLUE}Step 1: Git Status Check${NC}"
echo "─────────────────────────"
GIT_STATUS=$(git status --porcelain)
if [[ -n "$GIT_STATUS" ]]; then
    handle_error "Git working directory not clean" \
        "Commit or stash all changes before PROD deployment. Run: git status"
fi
...
if [ "$TODO_COUNT" -gt "10" ]; then
    show_warning "Too many TODOs: $TODO_COUNT (recommended max: 10 for PROD)"
else
    echo -e "${GREEN}✅ TODO count acceptable ($TODO_COUNT/10)${NC}"
fi
...
if [ "$CONSOLE_COUNT" -gt "0" ]; then
    handle_error "PROD requires ZERO console.log statements: $CONSOLE_COUNT (ZERO for PROD)"
else
    echo -e "${GREEN}✅ Console.log count acceptable ($CONSOLE_COUNT (PROD requires 0))${NC}"
fi
...
# Step 7: Release Notes Check (PROD requires release notes)
echo -e "${BLUE}Step 7: Release Notes Check${NC}"
echo "────────────────────────────"
RELEASE_NOTES_FILE="docs/RELEASE_NOTES.md"
if [ ! -f "$RELEASE_NOTES_FILE" ]; then
    handle_error "Release notes not found" \
        "Create $RELEASE_NOTES_FILE before PROD deployment"
fi

# Warn if release notes not updated recently
if ! find "$RELEASE_NOTES_FILE" -mtime -7 2>/dev/null | grep -q .; then
    show_warning "Release notes not updated in last 7 days"
fi
echo -e "${GREEN}✅ Release notes file exists${NC}"

# Screenshot reminder
echo
echo -e "${YELLOW}⚠️  Screenshot Reminder for PROD:${NC}"
echo "   iOS: 6.7\", 6.5\", 5.5\", iPad Pro (MANDATORY)"
echo "   Android: Phone, 7\" tablet, 10\" tablet (MANDATORY)"
echo
read -p "Continue to PROD deployment? (y/N): " -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
...
if [[ "$PLATFORM" == "ios" || "$PLATFORM" == "both" ]]; then
    echo -e "${CYAN}═══════════════════════════════════════════════════════${NC}"
    echo -e "${CYAN}    iOS PROD BUILD & UPLOAD                          ${NC}"
    echo -e "${CYAN}═══════════════════════════════════════════════════════${NC}"
    echo

    echo -e "${YELLOW}📱 Building and uploading iOS to App Store...${NC}"
    ...
    echo -e "${GREEN}✅ iOS uploaded to App Store${NC}"
    echo
fi
...
if [[ "$PLATFORM" == "android" || "$PLATFORM" == "both" ]]; then
    echo -e "${CYAN}═══════════════════════════════════════════════════════${NC}"
    echo -e "${CYAN}    ANDROID PROD BUILD & UPLOAD                       ${NC}"
    echo -e "${CYAN}═══════════════════════════════════════════════════════${NC}"
    echo

    echo -e "${YELLOW}📱 Building and uploading Android to Play Store Production...${NC}"
    ...
    echo -e "${GREEN}✅ Android uploaded to Play Store Production${NC}"
    echo
fi
...
# Commit
COMMIT_MSG="chore: Mobile PROD v${MARKETING_VERSION} (build ${IOS_BUILD_NUMBER})

Deployed to App Store / Play Store Production
...
# Create git tag
TAG_NAME="mobile-v${MARKETING_VERSION}-prod-${IOS_BUILD_NUMBER}"
git tag -a "$TAG_NAME" -m "Mobile PROD release v${MARKETING_VERSION} (build ${IOS_BUILD_NUMBER})"
...
if [[ "$PLATFORM" == "ios" || "$PLATFORM" == "both" ]]; then
    ...
    echo "  📱 App Store Testing group will receive notification"
    echo
fi
...
if [[ "$PLATFORM" == "android" || "$PLATFORM" == "both" ]]; then
    ...
    echo "  📱 Play Store Production Testing track is live"
    echo
fi

echo "Next Steps:"
echo "──────────"
echo "1. Collect feedback from PROD with internal team"
echo "2. Verify all features work correctly"
echo "3. When ready for external beta, run: ./scripts/deploy-mobile-prod.sh"
echo
echo -e "${CYAN}═══════════════════════════════════════════════════════${NC}"
